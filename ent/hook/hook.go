...
// The UserStoragePoolFunc type is an adapter to allow the use of ordinary
// function as UserStoragePool mutator.
type UserStoragePoolFunc func(context.Context, *ent.UserStoragePoolMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f UserStoragePoolFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.UserStoragePoolMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.UserStoragePoolMutation", m)
}
